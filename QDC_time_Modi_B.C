...
void QDC_time_Modi_B()
{
	
	const int number = 11;// change
  //gStyle -> SetOptFit();
  gStyle -> SetOptStat(0);
  ...
  TFile* file1 = new TFile(Form("Bar%iQDC_value_B.root",number),"read");
  TTree* treepeak = (TTree*)file1 -> Get("fitPeak");
  double peakvalue[25];
  treepeak -> SetBranchAddress("peakvalue",peakvalue);
  ...
  { 
	...
	QDC_Pos[i] = new TH2D(Form("QDC_Pos%d",i),Form("QDC vs Position %d Bar",i),150,-150,150,60,0,120);
	//QDC_Pos[i] = new TH2D(Form("QDC_Pos%d",i),Form("QDC vs Position %d Bar",i),200,-200,200,4000,0,8000);
  }  
  ///////////////////////////////////////////////////////////////// 
	TFile* OutPut = new TFile(Form("Bar%iCali_B.root",number),"recreate");
  /* TCanvas*  can1[6];
	 for(int i = 0 ; i < 6 ; i++)
	 { 
...
TCanvas* c1 = new TCanvas("c1","",700,700);
  ...
  for(int i = 0 ; i < 25 ; i++)
  {
	qdcPos[i] = new TH1D(Form("qdcPos%d",i),Form("qdc_position From %d to %d [cm]",-100+(i*8),-100+((i+1)*8) ),100,0,1000);
  }  
			  ...
			  if( 56 <= Channel[i] &&  Channel[i] <= 79 && 80 <= Channel[j] &&  Channel[j] <= 103)
			  {
				if(Channel[i] +24 == Channel[j])
				{
				  ...
				  for(int h = 0 ; h < 24 ; h++)
				  {
					...
					//QDC_Pos[h] -> Fill(Modi_Cross_diff[h],QDC_Geo[h]) ;
				  }

				  for(int k = 0 ; k < 25; k++)
				  {
				  	treepeak -> GetEntry(k);
					if( checkP[k] <= Modi_Cross_diff[number] && Modi_Cross_diff[number] <= checkP[k+1])
					{ 
					  Modi_QDC_Geo[number] = ((QDC_Geo[number]/peakvalue[k])*11.92); 
					}
				  }
				
					QDC_Pos[number] -> Fill(Modi_Cross_diff[number],Modi_QDC_Geo[number]) ;
  ...
  c1 -> cd();
  //QDC_Pos[13] -> Fit("pol2","","",-90,90,17,27);
  QDC_Pos[number] -> Draw("colz");
 // c1 -> SaveAs(Form("Bar%iCaliTest.pdf",number));
  QDC_Pos[number] -> Write();
}
